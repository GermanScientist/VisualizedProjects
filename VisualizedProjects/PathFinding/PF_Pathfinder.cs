...
namespace VisualizedProjects.Visualizer {
    namespace VisualizedProjects.FiniteStateMachine {
        namespace VisualizedProjects.PathFinding {
            class PF_Pathfinder : V_ProgramBase {
                //Properties
                private List<List<PF_Cell>> grid; //The grid list
                private List<PF_Cell> openSet; //Stores a list of nodes that still need to be evaluated
                private List<PF_Cell> closedSet; //Stores a list of all the nodes that have finished being evaluated
                private List<PF_Cell> finalPath; //The final (shortest) path after the algorythm has finished
                ...
                private PF_Cell startCell; //The starting point of the pathfinding search
                private PF_Cell finishCell; //The finishing point of the pathfinding search

                //The constructor
                public PF_Pathfinder() {
                    ...
                    grid = new List<List<PF_Cell>>(columnsAmount);
                    for (int i = 0; i < columnsAmount; i++) {
                        grid.Add(new List<PF_Cell>(rowsAmount));
                        for (int j = 0; j < columnsAmount; j++)
                            grid[i].Add(new PF_Cell(i, j, cellWidth, cellHeight)); //Cell(x, y, width, height)
                    }

                    //Create empty lists
                    openSet = new List<PF_Cell>(); //The open set
                    closedSet = new List<PF_Cell>(); //The closed set
                    finalPath = new List<PF_Cell>(); //The final path after the algorythm has finished
                ...
                private void FindShortestPath() {
                    ...
                    if (openSet.Count > 0) {
                        ...
                        PF_Cell currentCell = openSet[lowestIndex]; //Set the winning cell as the current cell, as it is first in the list
                        ...
                        if (currentCell == finishCell) {
                            ...
                            PF_Cell temporaryCell = currentCell;
                          ...
                          //If the current cell isn't the finish cell, the search hasn't ended yet
                            ...
                            List<PF_Cell> neighbors = currentCell.Neighbors; //Get the neighbors of the current cell
                            for (var i = 0; i < neighbors.Count; i++) {
                                PF_Cell neighbor = neighbors[i]; //Get the current neighbor
                                ...
                                if (!closedSet.Contains(neighbor) && neighbor.Walkable) {
                                    ...
                                    System.Numerics.Vector2 currentCellPosition = new System.Numerics.Vector2(currentCell.X, currentCell.Y);
                                    System.Numerics.Vector2 neighborPosition = new System.Numerics.Vector2(neighbor.X, neighbor.Y);
                                    int distanceBetweenCells = (int)System.Numerics.Vector2.Distance(currentCellPosition, neighborPosition);
                ...
                private int CalculateHeuristic(PF_Cell _neighbor, PF_Cell _finishCell) {
